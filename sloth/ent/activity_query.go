...
func (aq *ActivityQuery) QueryOwner() *UserQuery {
...
func (aq *ActivityQuery) WithOwner(opts ...func(*UserQuery)) *ActivityQuery {
	query := &UserQuery{config: aq.config}
...
//		GroupBy(activity.FieldStartedAt).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
//
...
//
//	client.Activity.Query().
//		Select(activity.FieldStartedAt).
//		Scan(ctx, &v)
//
...
func (aq *ActivityQuery) sqlAll(ctx context.Context) ([]*Activity, error) {
	...
	if query := aq.withOwner; query != nil {
		...
		for i := range nodes {
			if fk := nodes[i].user_owner; fk != nil {
				ids = append(ids, *fk)
		...
		for _, n := range neighbors {
			...
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "user_owner" returned %v`, n.ID)
			}
			for i := range nodes {
