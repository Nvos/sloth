...
func (uq *UserQuery) QueryOwner() *ActivityQuery {
...
func (uq *UserQuery) WithOwner(opts ...func(*ActivityQuery)) *UserQuery {
	query := &ActivityQuery{config: uq.config}
...
func (uq *UserQuery) sqlAll(ctx context.Context) ([]*User, error) {
	...
	if query := uq.withOwner; query != nil {
		...
		for _, n := range neighbors {
			fk := n.user_owner
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "user_owner" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "user_owner" returned %v for node %v`, *fk, n.ID)
			}
